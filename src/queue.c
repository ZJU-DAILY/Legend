...
void nvm_queue_clear(nvm_queue_t* queue, const nvm_ctrl_t* ctrl, bool cq, uint16_t no, void* vaddr, uint64_t ioaddr)
{
    ...
    // queue->max_entries = 256;
    queue->current_count = 0;
}

void nvm_queue_clear_our(nvm_queue_t* queue, const nvm_ctrl_t* ctrl, bool cq, uint16_t no, void* vaddr, uint64_t ioaddr, size_t q_size)
{
    queue->no = no;
    // queue->max_entries = 0;
    queue->entry_size = cq ? sizeof(nvm_cpl_t) : sizeof(nvm_cmd_t);
