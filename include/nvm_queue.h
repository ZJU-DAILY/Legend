...
// #include </usr/local/cuda-12.4/include/cuda_runtime.h>
...
__host__
void nvm_queue_clear_our(nvm_queue_t* q,            // NVM queue descriptor
                     const nvm_ctrl_t* ctrl,    // NVM controller handle
                     bool cq,                   // Is this a completion queue or submission queue?
                     uint16_t no,               // Queue number
                     void* vaddr,               // Virtual address to queue memory
...
__device__ static inline
void nvm_sq_enqueue_n_our(nvm_queue_t* sq, nvm_cmd_t* cmd, uint32_t n, uint16_t threadNum)
{
    ulonglong4* start = (ulonglong4 *)(((nvm_cmd_t *) sq->vaddr) + ((sq->head + (uint32_t)threadNum) & ((uint32_t)sq->max_entries - 1)));
    ulonglong4* cmd_dw = (ulonglong4 *)(cmd->dword);
    #pragma unroll
    for(uint32_t i = 0; i < 64 / sizeof(ulonglong4); i++){
        start[i] = cmd_dw[i];
    }
    if(threadNum == 0){
        sq->tail = (sq->tail + n) & ((uint32_t)sq->max_entries - 1);
        *((volatile uint32_t*) sq->db) = sq->tail;
        // printf("%d %d\n", sq->tail, sq->max_entries);
    }
}
#endif
...
nvm_cpl_t* nvm_cq_poll(const nvm_queue_t* cq)
{
    ...
    // if((((cpl->dword[3] & 0x00010000) >> 16) ^ cq->phase) == 1)
    {
        return NULL;
    }
