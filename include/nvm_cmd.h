...
__host__ __device__ static inline
void nvm_cmd_data_our(nvm_cmd_t* cmd, uint64_t* prp1, uint64_t* prp2, uint32_t threadNum)
{
    uint64_t prp1_entry = prp1[threadNum];
    uint64_t prp2_entry = 0;
    if(prp2 != NULL){
        prp2_entry = prp2[threadNum];
    }
    // printf("%lx %d\n", prp1_entry, threadNum);
    nvm_cmd_data_ptr(cmd, prp1_entry, prp2_entry);
    // return prp;
}
