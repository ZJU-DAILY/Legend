...
void Settings::parseArguments(int argc, char** argv)
{
    OptionMap parsers = {
        ...
        {'t', OptionPtr(new Range(numThreads, 1, 16384, "threads", "number of CUDA threads", "32"))},
        ...
        {'P', OptionPtr(new Range(pageSize, 1, (uint64_t)std::numeric_limits<uint64_t>::max, "page_size", "size of pages", "4096"))},
        {'Q', OptionPtr(new Range(queuePairs, 1, 12, "queue_pairs", "number of qps", "1"))},
        {'D', OptionPtr(new Range(queueDepth, 1, 16384, "queue_depth", "depth of queue", "1024"))},
    };
